...
To perform LFMM, genomic and climatic data needs to be in allele count so at the individual level and not population.  
```{r load data}
...
Run_LFMM2 <- lfmm2(input = "C:/Users/tfrancisco/Documents/Thèse/Data/Espèces/Taxus_baccata/genetic_new/genomic_data.lfmm", env = climatic_data_475[,-c(1:3)], K = 2, effect.sizes = T)
...
score_Pca_imputed <- data.frame(score_LFMM, row.names (genomic_data))
      colnames(score_Pca_imputed) <- c(paste0("PC1"), paste0("PC2"), "VCF_ID")
      #add pop info on the score_pca
  score_Pca_imputed_meta <- merge(climatic_data_475[,c(1:2)],score_Pca_imputed, "VCF_ID")
      
      # add country, population information
      PCa_df_imputed <- merge(score_Pca_imputed_meta, meta_data_pop, "Population")
      
      # genetic PCA
      ggplot_representation <- ggplot() +
        geom_point(data = PCa_df_imputed, aes(PCa_df_imputed[,3],PCa_df_imputed[,4], color = PCa_df_imputed$Country)) +
...
abline(h = -log10(0.05/8616), lty = 2, col = "orange")
